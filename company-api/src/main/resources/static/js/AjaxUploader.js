...
AjaxProcesser.prototype.reset=function(a){
    ...
    this.processIng.innerHTML="select file...";
	return;
};
...
AjaxProcesser.prototype.createUploader=function(){
	...
	processIng.style.cssText="width:340px;font-size:9pt;height:13px;border:0px #005EAC solid;margin:0px;padding:3px 0px 2px 5px;background-color:#6D84B4;color:#ffffff;font-weight:bold;";
     var _ThisClose=document.createElement("span");
     _ThisClose.style.cssText="float:right;margin-right:5px;cursor:pointer;";
     _ThisClose.innerHTML="close";
	...
	file.name="file" + (++this.files.count);
	file.size=40;
	if(!this.defaultStyle){file.style.cssText="width:280px;height:20px;border:1px #aaaaaa solid;padding:3px 0px 1px 0px;margin:3px;";}
	this.files[file.name]=file; //添加到文件集合
	...
	button.value="upfile";
	button.type="button";
	if(!this.defaultStyle){button.style.cssText="width:50px;height:20px;border:1px #aaaaaa solid;padding:3px 0px 1px 0px;margin:3px;";}
	...
	processDiv.style.cssText="display:none;padding:3px;font-size:9pt;border:1px #eeeeee solid;width:330px;margin:5px 2px 2px 0px;height:auto;";
	var processBar=document.createElement("div");//创建一个进度条
	processBar.style.cssText="font-size:9pt;width:330px;padding:0px;margin:0px;height:auto;border:1px #dddddd solid;background-color:#ffffff;";
...
AjaxProcesser.prototype.getProcess=function(){
    ...
    _.Ajax({
        asc:false,
 	    method	: "get",
	    dataType: "json",
 	    url	    : "file/getProcess.asp?processid=" + this.processID + "&rnd=" + Math.random(),
 	    data	: "",
		...
		if(msg==null){return;}
            ...
            str= str + "size:" + reSize(pro.total);
            str= str + "&nbsp; <span style=\"color:green;\">up:" + reSize(pro.cur) + "</span>";
			str= str + "&nbsp; <span style=\"color:red;\">up:" + pro.speed + "</span>";;
            _this.processInfo.innerHTML= str;
            _this.process.innerHTML=pro.percent;
            _this.process.style.width=Math.floor(330 * pro.process) + "px"; //显示进度
            if(pro.finish){
				_this.frm.reset();
